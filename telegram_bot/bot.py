...
# Используйте файл telegram_bot/.env (добавлен в .gitignore) или задавайте переменные окружения в системе/CI.
...
async def get_chat(user_id):
    """Возвращает/создаёт чат клиента GenAI для пользователя.
    Поддерживаем как синхронный, так и асинхронный API библиотеки (если метод возвращает coroutine).
    """
    if user_id not in user_chats:
        created = client.chats.create(model="gemini-3.5-flash")
        if asyncio.iscoroutine(created):
            user_chats[user_id] = await created
        else:
            user_chats[user_id] = created
...
async def handle_message(message: Message):
    ...
    try:
        maybe_response = chat.send_message(contents, config=config)
        ...
        if reply_text is None:
            # Попытка извлечь текст из возможных полей или привести объект к строке
            reply_text = getattr(response, "message", None) or getattr(response, "candidates", None)
            if isinstance(reply_text, (list, tuple)):
                reply_text = "\n".join(map(str, reply_text))
            if reply_text is None:
